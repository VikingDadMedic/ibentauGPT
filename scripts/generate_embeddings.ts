...
{
	const textSplitter = new MarkdownTextSplitter( { chunkSize: 1000, chunkOverlap: 20 } );
	...
	await PineconeStore.fromTexts( embeddings, [], embeddingModel, { pineconeIndex } );
...
async function scrape_researchr_page ( url: string, browser: Browser ): Promise<string>
{
	...
	await page.goto( url );
	const element = await page.waitForSelector( '.content-container', { timeout: 100 } );
	if ( !element )
	{
	...
	{
		const elements = element.querySelectorAll( '*:not(p, h1, h2, h3, h4, h5, h6, li, blockquote, pre, code, table, dl, div)' );
